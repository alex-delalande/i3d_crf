...
class CRF(nn.Module):
    ...
    def __init__(self,
        ...
        self.mat = nn.Parameter((1/np.sqrt(num_classes))*torch.randn(num_classes,num_classes))
        self.bias = nn.Parameter((1/np.sqrt(num_classes))*torch.randn(num_classes))


    def forward(self, x):

        x = x.permute(0,2,1)
        x = torch.matmul(x, self.mat)  + self.bias
        x = x.permute(0,2,1)
