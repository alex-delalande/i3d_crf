...
import recall_calculator as rec_calculator
...
def calculate_map(predictions, actuals, top_k=20):
  """Performs a local (numpy) calculation of the mean average precision.
  ...
  """
  map_calculator_loop = map_calculator.MeanAveragePrecisionCalculator(predictions.shape[1])
  sparse_predictions, sparse_labels, num_positives = top_k_by_class(predictions, actuals, top_k)
  map_calculator_loop.accumulate(sparse_predictions, sparse_labels, num_positives)
  aps = map_calculator_loop.peek_map_at_n()
  return numpy.mean(aps)


def calculate_recall(predictions, actuals, top_k=20):
  """Performs a local (numpy) calculation of the recall at k.
  ...
  Returns:
    float: The global average precision.
  """
  recall_calculator = rec_calculator.RecallCalculator()
  sparse_predictions, sparse_labels, num_positives = top_k_by_class(predictions, actuals, top_k)
  recall_calculator.accumulate(flatten(sparse_predictions), flatten(sparse_labels), sum(num_positives))
  return recall_calculator.peek_ap_at_n()
...
class EvaluationMetrics(object):
  ...
  def __init__(self, num_class, top_k):
    ...
    self.global_recall_calculator = rec_calculator.RecallCalculator()
    self.top_k = top_k
    self.num_examples = 0
  ...
  def accumulate(self, predictions, labels, loss):
    ...
    self.global_recall_calculator.accumulate(flatten(sparse_predictions), flatten(sparse_labels), sum(num_positives))
  ...
  def get(self):
    ...
    recall = self.global_recall_calculator.peek_ap_at_n()

    epoch_info_dict = {}
    return {"avg_hit_at_one": avg_hit_at_one, "avg_perr": avg_perr,
            "avg_loss": avg_loss, "aps": aps, "gap": gap, "recall": recall}
